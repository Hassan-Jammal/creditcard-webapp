<template>
  <div class="w-full space-y-8">
    ...
    <div class="p-4 xl:p-8 rounded-3xl bg-[#F7F7F7]">
      ...
      <label class="custom-radio flex gap-4 mt-4 text-gray-500">
        <input
          ...
          value="Yes"
          v-model="form.get_started_is_acknowledged"
        />
        <span></span>
        <p>
      ...
      <div v-if="touched.get_started_is_acknowledged && errors.get_started_is_acknowledged" class="text-xs text-red-500">{{ errors.get_started_is_acknowledged }}</div>
    </div>
    ...
    <div class="p-4 xl:p-8 pb-8 xl:pb-16 rounded-3xl bg-[#F7F7F7]">
      ...
      <div class="mt-16">
        <RangeSlider
          v-if="chosenCard?.slider"
          :key="chosenCard.id"
          :card-id="chosenCard.id"
          :min="chosenCard.slider.min"
          :max="chosenCard.slider.max"
          :steps="chosenCard.slider.steps"
          :initialTo="chosenCard.slider.initialTo"
          :scaleStep="chosenCard.slider.scaleStep"
          :currency="chosenCard.currency"
          v-model="selectedRange"
        />

        <div v-if="errors.get_started_credit_card_limit" class="text-xs text-red-500">{{ errors.get_started_credit_card_limit }}</div>
      </div>
    </div>
  </div>
</template>
